...
export function OnboardingModal({ open, onComplete }: OnboardingModalProps) {
  ...
  // Auto-advance from Jobraker Recruiter sign-in step when OAuth completes
  useEffect(() => {
    if (onboardingPath !== 'jobraker-recruiter' || currentStep !== 0) return
  ...
  const renderStepIndicator = () => {
    // Jobraker Recruiter path: Sign In (0), Connect (3), Done (4) = 3 dots
    // BYOK path: Sign In (0), Upsell (1), Model (2), Connect (3), Done (4) = 5 dots
    // Before path is chosen: show 3 dots (minimal)
    const jobrakerRecruiterSteps = [0, 3, 4]
    const byokSteps = [0, 1, 2, 3, 4]
    const steps = onboardingPath === 'byok' ? byokSteps : jobrakerRecruiterSteps
          ...
          <div className="flex flex-col items-center gap-4 w-full max-w-xs">
            <Button
              onClick={() => {
                ...
                startConnect('jobraker-recruiter')
              }}
              size="lg"
              className="w-full"
  ...
  // Step 1: BYOK upsell — explain benefits of Jobraker Recruiter before continuing with BYOK
  const renderByokUpsellStep = () => (
    <div className="flex flex-col">
      <DialogHeader className="text-center mb-6">
        <DialogTitle className="text-2xl">Before you continue</DialogTitle>
        <DialogDescription className="text-base max-w-md mx-auto">
          With a Jobraker Recruiter account, you get:
        </DialogDescription>
      </DialogHeader>
      ...
      <p className="text-sm text-muted-foreground text-center mb-6">
        By continuing, you'll set up your own API keys instead of using Jobraker Recruiter's managed gateway.
      </p>
