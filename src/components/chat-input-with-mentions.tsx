  ...
  const loadModelConfig = useCallback(async () => {
    try {
      if (isJobrakerRecruiterConnected) {
        ...
        const jobrakerRecruiterProvider = listResult.providers?.find(
          (p: { id: string }) => p.id === 'jobraker-recruiter'
        )
        const models: ConfiguredModel[] = (jobrakerRecruiterProvider?.models || []).map(
          (m: { id: string }) => ({ provider: 'jobraker-recruiter', model: m.id })
        )
