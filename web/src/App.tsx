...
const GRAPH_TAB_PATH = '__jobraker-recruiter_graph_view__'
const SUGGESTED_TOPICS_TAB_PATH = '__jobraker-recruiter_suggested_topics__'
const MEETINGS_TAB_PATH = '__jobraker-recruiter_meetings__'
const LIVE_NOTES_TAB_PATH = '__jobraker-recruiter_live_notes__'
const BG_TASKS_TAB_PATH = '__jobraker-recruiter_bg_tasks__'
const EMAIL_TAB_PATH = '__jobraker-recruiter_email__'
const WORKSPACE_TAB_PATH = '__jobraker-recruiter_workspace__'
const WORKSPACE_ROOT = 'knowledge/Workspace'
const KNOWLEDGE_VIEW_TAB_PATH = '__jobraker-recruiter_knowledge_view__'
 ...
 *
 * - jobraker-recruiter:  jobraker-recruiter/2026-03-20/meeting-xxx.md  → keeps date folders as-is
...
/**
 * Parse a jobraker-recruiter:// deep link into a ViewState. Returns null if the URL is
 * malformed or names an unknown target.
 *
 * Shape: jobraker-recruiter://open?type=<file|chat|graph|task|suggested-topics|meetings|live-notes>&...
...
function parseDeepLink(input: string): ViewState | null {
  const SCHEME = 'jobraker-recruiter://'
...
function App() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jobraker-recruiter:open-copilot-edit-live-note', handler as EventListener)
    return () => window.removeEventListener('jobraker-recruiter:open-copilot-edit-live-note', handler as EventListener)
  }, [submitFromPalette])
  ...
  useEffect(() => {
    ...
    window.addEventListener('jobraker-recruiter:open-live-note-panel', handler as EventListener)
    return () => window.removeEventListener('jobraker-recruiter:open-live-note-panel', handler as EventListener)
  }, [])
  ...
  useEffect(() => {
    ...
    window.addEventListener('jobraker-recruiter:open-copilot-prompt', handler as EventListener)
  ...
  return (
    <TooltipProvider delayDuration={0}>
      <SidebarSectionProvider defaultSection="tasks" onSectionChange={(section) => {
        ...
        <div className="jobraker-recruiter-shell flex h-svh w-full overflow-hidden">
      ...
      <Dialog open={showMeetingPermissions} onOpenChange={setShowMeetingPermissions}>
        <DialogContent showCloseButton={false}>
          <DialogHeader>
            ...
            <DialogDescription>
              Jobraker Recruiter needs <strong>Screen Recording</strong> permission to capture meeting audio from other apps (Zoom, Meet, etc.). This feature won't work without it.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm text-muted-foreground">
            ...
            <ol className="list-decimal list-inside space-y-1.5">
              ...
              <li>Toggle on <strong>Jobraker Recruiter</strong></li>
